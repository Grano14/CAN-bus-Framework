...
forceinline void ascon_gettag(ascon_state_t* s, uint8_t* t) {
  /*
	Modifica 01: rimozione di queste due righe sistituite con la successiva per validare solo 4 byte del tag
	SQUEEZE(t, s->b[3], 8);
  	SQUEEZE(t + 8, s->b[4], 8);
		|
		V
...
forceinline int ascon_verify(ascon_state_t* s, const uint8_t* t) {
  ...
  /*
	Modifica 02: rimozione di queste due righe sistituite con la successiva per validare solo 4 byte del tag
	r |= VERIFY(s->b[3], t, 8);
